...
import { getFilters, getTimeSeriesData, getSummaryStats, getDateRange, getAllCategories } from '../services/api';
...
// ══════════════════════════════════════════════════════════════════════════════
function CategoryBlock({ categoryName, categoryData=[], categorySummary=[], plotType, selectedWindow, availableWindows, activeTab, isDark }) {
    ...
    const filteredChartData = useMemo(() => {
        ...
        return res;
    }, [categoryData, selectedWindow, availableWindows]);
    
    const visibleSummaryData = useMemo(() => sortedSummaryData.filter(s => selectedMeasures.has(s.measure)), [sortedSummaryData, selectedMeasures]);
    ...
    function buildMeasuresByUnit(sd) {
        const g = {};
        sd.forEach((s, i) => { const u = s.unit || 'Unknown'; if (!g[u]) g[u] = []; g[u].push({ ...s, color: COLORS[i % COLORS.length] }); });
        return g;
    }
    ...
    const piePerUnit = useMemo(() => {
        const g = {}; visibleSummaryData.filter(s => s.latest > 0).forEach((s, i) => { const u = s.unit || 'Unknown'; if (!g[u]) g[u] = []; g[u].push({ name: s.measure, value: s.latest, fill: COLORS[i % COLORS.length] }); });
        Object.values(g).forEach(a => a.sort((a, b) => b.value - a.value)); return g;
    }, [visibleSummaryData]);

    const meanPiePerUnit = useMemo(() => {
        const g = {}; visibleSummaryData.filter(s => s.average > 0).forEach((s, i) => { const u = s.unit || 'Unknown'; if (!g[u]) g[u] = []; g[u].push({ name: s.measure, value: s.average, fill: COLORS[i % COLORS.length] }); });
        Object.values(g).forEach(a => a.sort((a, b) => b.value - a.value)); return g;
    }, [visibleSummaryData]);
    ...
    function renderPlot(unit, measures, data) {
        ...
        return (
            <div key={unit} className="w-full h-[480px] p-4 pr-10 border border-slate-100 rounded-2xl bg-slate-50/50 mt-6">
                <ResponsiveContainer width="100%" height="100%">
                  <CC data={data} margin={{ top: 20, right: 20, left: 20, bottom: 20 }}>
                    <CartesianGrid strokeDasharray="4 4" vertical={false} stroke={gc} />
                    <XAxis dataKey="ageDays"
                      tickFormatter={t => `Day ${t != null ? t : ''}`}
                      tick={{ fontSize: 12, fill: ac, fontWeight: 700 }} tickMargin={14} axisLine={{ stroke: alc, strokeWidth: 1.5 }} tickLine={false}
                      interval="preserveStartEnd" textAnchor="middle" height={60} />
                    <YAxis yAxisId="left" orientation="left"
                      tick={{ fontSize: 12, fill: ac, fontWeight: 700 }} axisLine={false} tickLine={false} width={62}
                      domain={['auto', 'auto']} tickFormatter={fmtTick}
                      label={{ value: unit, angle: -90, position: 'insideLeft', offset: 10, style: { fontSize: 11, fill: ac, fontWeight: 700 } }} />
                    <Tooltip cursor={{ strokeDasharray: '4 4', strokeWidth: 1.5, stroke: alc }} content={<CustomTooltip unit={unit} />} />
                    <Legend wrapperStyle={{ paddingTop: 0, paddingBottom: 15 }} iconType="circle" iconSize={10} verticalAlign="top"
                      formatter={v => <span style={{ fontWeight: 700, fontSize: 13, color: '#334155' }}>{measureLabel(v)}</span>} />
                    {series}
                  </CC>
                </ResponsiveContainer>
            </div>
    ...
    return (
        <section className="bg-white dark:bg-slate-900 border border-slate-200 shadow-sm rounded-3xl mb-8 overflow-hidden">
            <div className="p-6 md:p-8 border-b border-slate-100">
                <h2 className="text-2xl md:text-3xl font-black text-slate-800 dark:text-slate-100 flex items-center gap-3 mb-6">
                ...
                {selectedMeasures.size === 0 ? (
                     ...
                     <>
                        ...
                        {activeTab === 'composition' && (() => {
                            ...
                            // MODIFIED: Breakdown logic follows pieViewMode
                            let breakdown = [];
                            if (pieViewMode === 'mean') {
                                ...
                                breakdown = measureCols.map(m => ({ 
                                    ...m, 
                                    value: selectedRow && selectedRow[m.key] != null ? selectedRow[m.key] : null 
                            ...
                            return (
                                <div>
                                    {Object.keys(activePieData).map(unit => {
                                        const slices = activePieData[unit], pieTotal = slices.reduce((s, e) => s + e.value, 0), pieMax = slices[0]?.value ?? 1;
                                        return (
                                            <div key={unit} className="grid grid-cols-1 xl:grid-cols-2 gap-6 items-start mt-6">
                                                ...
                                                <div className="bg-slate-50 dark:bg-slate-800/50 p-6 rounded-3xl border border-slate-200">
                                                    ...
                                                    <div className="overflow-y-auto pr-2" style={{ maxHeight: pieViewMode === 'mean' ? '400px' : '300px' }}>
                                                        ...
                                                        : breakdown.length === 0 ? (<div className="py-16 text-center text-slate-400 font-bold uppercase text-xs">No measurements found</div>)
                                                        : (breakdown.map((m, idx) => {
                                                            const maxValInList = breakdown.reduce((p, c) => Math.max(p, c.value), 0);
                                                            const pct = total > 0 ? (m.value / total) * 100 : 0, barW = maxValInList > 0 ? (m.value / maxValInList) * 100 : 0;
                                                            return (
                                                                <div key={m.key} className="mb-3">
                                                                    <div className="flex justify-between items-end mb-1">
                                                                        <span className="text-[13px] font-bold text-slate-700 dark:text-slate-300">{measureLabel(m.key)}</span>
                                                                        <div className="text-right">
                                                                            <span className="text-[13px] font-black text-slate-900 dark:text-slate-100">{Number(m.value).toFixed(2)}</span>
                                                                            <span className="text-[10px] text-slate-400 font-bold ml-1">{unit}</span>
                                                                            <span className="text-[11px] font-bold text-indigo-500 ml-2">({pct.toFixed(1)}%)</span>
                                                                        </div>
                                                                    </div>
                                                                    <div className="h-2 rounded-full bg-slate-200 dark:bg-slate-700 overflow-hidden"><div className="h-full rounded-full transition-all" style={{ width: `${barW}%`, backgroundColor: m.color }} /></div>
                                                                </div>
                                                            );
                                                        }))}
                                                    </div>
...
export default function Dashboard() {
  ...
  useEffect(() => {
    ...
    if (selectionProfile.isExclusivelyPlant) {
      if (selectedSoil !== 'All Soils') setSelectedSoil('All Soils');
    }
  }, [selectionProfile, selectedCrop, selectedSoil]);
  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-slate-950 font-sans relative overflow-x-hidden transition-colors">
      <div className="p-4 md:p-8 max-w-[1920px] mx-auto pb-24">

        {/* ── HEADER ── */}
        <header className="mb-8 flex flex-col xl:flex-row justify-between items-center bg-white dark:bg-slate-900 p-6 md:p-8 rounded-3xl shadow-[0_4px_24px_-8px_rgba(0,0,0,0.05)] border border-slate-200 gap-6 relative z-50">
          <div>
            <h1 className="text-3xl md:text-5xl font-black tracking-tight text-slate-900 dark:text-slate-100 flex items-center gap-4">
              <Leaf className="w-10 h-10 text-emerald-500 shrink-0" /> Soil Analytics
            </h1>
            <p className="mt-3 text-slate-500 font-bold tracking-wide">Multi-Category Dashboard</p>
          </div>
          <div className="flex flex-wrap justify-end items-end gap-5 w-full xl:w-auto">
            <button onClick={() => setIsDark(!isDark)}
              className="h-[46px] w-[46px] flex-shrink-0 flex items-center justify-center rounded-xl bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 text-slate-500 hover:border-slate-300 transition-colors">
              {isDark ? <Sun className="w-5 h-5 text-amber-500" /> : <Moon className="w-5 h-5 text-indigo-500" />}
            </button>

            {/* Custom Multi-Select Categories */}
            <div className="flex flex-col gap-1 flex-1 sm:flex-none sm:w-[260px]" ref={dropdownRef}>
              <label className="text-[10px] font-black text-indigo-500 flex items-center justify-between uppercase tracking-widest px-1">
                 <span>Categories Tested</span>
                 <span className="text-indigo-600 bg-indigo-100 px-1.5 py-0.5 rounded">{selectedCategories.length}</span>
              </label>
              <div className="relative">
                <button 
                  onClick={() => setIsDropdownOpen(!isDropdownOpen)}
                  className="w-full flex items-center justify-between bg-white dark:bg-slate-800 border-2 border-indigo-200 text-slate-700 dark:text-slate-300 py-2 px-3.5 rounded-xl text-[13px] font-bold shadow-sm hover:border-indigo-400 transition-colors h-[42px]">
                  <span className="truncate pr-2">{selectedCategories.length === 0 ? 'Select categories' : selectedCategories.length === 1 ? selectedCategories[0] : `${selectedCategories.length} Selected`}</span>
                  <ChevronDown className="w-4 h-4 text-indigo-400 flex-shrink-0" />
                </button>
                
                {isDropdownOpen && (
                  <div className="absolute top-full left-0 right-0 sm:w-[320px] mt-2 bg-white dark:bg-slate-900 border border-slate-200 shadow-2xl rounded-2xl py-3 px-2 z-50 max-h-[400px] overflow-y-auto flex flex-col gap-1">
            ...
            <div className="flex flex-col gap-1 w-1/2 sm:w-auto">
              <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest px-1 flex items-center gap-1"><LineChartIcon className="w-3.5 h-3.5" /> Chart Type</label>
              <div className="relative">
                <select value={plotType} onChange={e => setPlotType(e.target.value)}
                  className="appearance-none h-[42px] bg-slate-50/50 dark:bg-slate-900 border border-slate-200 text-slate-700 dark:text-slate-300 py-2 pl-3.5 pr-8 rounded-xl focus:outline-none focus:ring-2 focus:ring-slate-500/20 font-bold hover:bg-slate-50 cursor-pointer text-[13px] transition-colors">
            ...
            <div className="flex flex-col gap-1 w-full sm:w-[170px]">
              <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest px-1 flex items-center gap-1"><Calendar className="w-3.5 h-3.5" /> Timeline Focus</label>
              <div className="relative">
                <select value={selectedWindow} onChange={e => setSelectedWindow(e.target.value)}
                  className="appearance-none h-[42px] w-full bg-slate-50/50 dark:bg-slate-900 border border-slate-200 text-slate-700 dark:text-slate-300 py-2 pl-3.5 pr-8 rounded-xl focus:outline-none focus:ring-2 focus:ring-slate-500/20 font-bold hover:bg-slate-50 cursor-pointer text-[13px] transition-colors">
            ...
            {/* crop */}
            <div className={`flex flex-col gap-1 w-1/2 sm:w-[130px] transition-opacity duration-300 ${selectionProfile.isExclusivelySoil ? 'opacity-40 cursor-not-allowed pointer-events-none' : ''}`}>
              ...
              <div className="relative">
                <select value={selectedCrop} onChange={e => setSelectedCrop(e.target.value)} disabled={selectionProfile.isExclusivelySoil}
                  className="appearance-none h-[42px] w-full bg-emerald-50 dark:bg-emerald-900/30 border border-emerald-200 text-emerald-800 dark:text-emerald-300 py-2 pl-3.5 pr-8 rounded-xl focus:outline-none focus:ring-2 focus:ring-slate-500/20 font-bold cursor-pointer text-[13px] transition-colors disabled:bg-slate-100 disabled:border-slate-200 disabled:text-slate-400">
            ...
            {/* soil */}
            <div className={`flex flex-col gap-1 w-1/2 sm:w-[130px] transition-opacity duration-300 ${selectionProfile.isExclusivelyPlant ? 'opacity-40 cursor-not-allowed pointer-events-none' : ''}`}>
              <label className="text-[10px] font-black text-amber-500 uppercase tracking-widest px-1 flex items-center gap-1">
                Soil Type {selectionProfile.isExclusivelyPlant && <Box className="w-3 h-3" />}
              </label>
              <div className="relative">
                <select value={selectedSoil} onChange={e => setSelectedSoil(e.target.value)} disabled={selectionProfile.isExclusivelyPlant}
                  className="appearance-none h-[42px] w-full bg-amber-50 dark:bg-amber-900/30 border border-amber-200 text-amber-800 dark:text-amber-300 py-2 pl-3.5 pr-8 rounded-xl focus:outline-none focus:ring-2 focus:ring-slate-500/20 font-bold cursor-pointer text-[13px] transition-colors disabled:bg-slate-100 disabled:border-slate-200 disabled:text-slate-400">
                  {filters.soil_types.map(s => <option key={s} value={s}>{s}</option>)}
                </select>
                <ChevronDown className="pointer-events-none absolute right-3 top-1/2 -translate-y-1/2 w-4 h-4 text-amber-500" />
              </div>
            </div>
          </div>
        </header>
        ...
        {/* ── STICKY TOOLBAR ── */}
        <div className="flex overflow-x-auto bg-white dark:bg-slate-900 p-2 rounded-2xl shadow-sm border border-slate-200 mb-8 sticky top-4 z-40 gap-2">
            {[
              { key: 'summary', label: 'Insight Cards', icon: <LayoutDashboard className="w-5 h-5" /> },
              { key: 'trends', label: 'Trajectory Plots', icon: <TrendingUp className="w-5 h-5" /> },
              { key: 'composition', label: 'Composition Split', icon: <PieChartIcon className="w-5 h-5" /> },
            ].map(tab => (
              <button key={tab.key} onClick={() => setActiveTab(tab.key)}
                className={`flex flex-1 justify-center items-center gap-2 px-6 py-3.5 rounded-xl font-bold text-[15px] transition-all whitespace-nowrap
                  ${activeTab === tab.key ? 'bg-slate-900 text-white dark:bg-indigo-600 shadow' : 'text-slate-500 hover:bg-slate-100 dark:hover:bg-slate-800 hover:text-slate-800 dark:hover:text-slate-200'}`}>
                {tab.icon} {tab.label}
              </button>
            ))}
        </div>
